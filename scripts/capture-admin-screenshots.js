...
async function captureScreenshots() {
  ...
  try {
    ...
    if (!url.includes('/login')) {
      ...
      // 16. Create New Page (to show form)
      console.log('16. Capturing Create Page form...');
      await page.goto('http://localhost:3003/admin/collections/pages', { waitUntil: 'networkidle' });
      await page.waitForTimeout(2000);
      
      // Click Create New button
      const createBtn = await page.$('a[href*="/create"], button:has-text("Create new")');
      if (createBtn) {
        ...
        await page.screenshot({ 
          path: path.join(screenshotsDir, 'page-create-form.png'),
          fullPage: false
        });
      }
