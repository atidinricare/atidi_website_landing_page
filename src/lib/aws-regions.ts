/**
 * AWS region options shared by the S3 storage and CloudFront CDN selects in
 * Site Settings. Both dropdowns must stay in sync with the Postgres enums
 * `enum_site_settings_storage_settings_s3_region` and
 * `enum_site_settings_cdn_settings_cloudfront_region` — adding a region here
 * also needs a migration that adds the value to both enums.
 */
